...
if manifest.get('asset_revision')!=2: raise RuntimeError('Reviewed logo revision required')
...
for f in res.glob('wallet_*.webp'):
    stem=f.stem.removesuffix('_card')
    if stem not in used: f.unlink()
...
图形按原始宽高比缩放，去除已知的外层图标底板，不拉伸。卡面使用透明单色标识以保持对比度，列表保留品牌色；完整许可证随应用打包在 assets/licenses，商标归各自权利人所有。不同 Logo 形状会有不同占用比例，不强行填满同一个矩形。

名称匹配支持大小写、常见中英文别名、部分繁简体和公司后缀归一化；精确匹配优先、歧义回退，结果使用有界缓存。卡组织识别只根据本地号码前缀提供显示提示，不是完整 BIN 查询或卡片有效性验证。
''')
print('Visual regression API references and unused feature logo assets cleaned.')
