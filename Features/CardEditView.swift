...
struct CardEditView: View {
    ...
    private var cardMediaSection: some View {
        Section("卡片媒体文件") {
            ...
            if cardImages.isEmpty {
                ...
                Text("共 \(cardImages.count) 张图片 · 附件总大小 \(formatFileSize(cardImages.reduce(0) { $0 + imageByteSize($1) }))")
                    .font(.footnote)
                    .foregroundColor(.secondary)
    ...
    private func applyScanResult(_ result: CardScanResult, imageAsset: CardImageAsset?) {
        ...
        if let matchedLevel = result.level, level.isEmpty {
            level = CardLevelGroup.normalize(matchedLevel)
...
private struct CardLevelPickerField: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 8) {
                Label("卡级别", systemImage: "crown.fill")
                Spacer()
