...
enum LocalSyncHistory {
    private static var file: URL { FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0].appendingPathComponent("CardWallet/sync-history.enc") }
    static func read<T: Codable>(_ type: T.Type, legacyKey: String) throws -> T? {
        if FileManager.default.fileExists(atPath: file.path) {
            let data = try Data(contentsOf: file)
            let plain = try CryptoManager.decryptLocalData(data)
